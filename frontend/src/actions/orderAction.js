...
export const getOrderedDrinkCount = () => async (dispatch) =>{
    dispatch({
        type: COUNT_ORDERED_DRINK_REQUEST
    });
    try {
        const {data} = await axios.get(`/api/admin/order/list/drink/count`);
        dispatch({type: COUNT_ORDERED_DRINK_SUCCESSFUL, payload: data});
    } catch (error) {
        dispatch({type: COUNT_ORDERED_DRINK_FAILED, 
            payload: error.response 
            && error.response.data.message 
            ? error.response.data.message
            : error.message,});
...
export const listOfOrdersIncomePerMonth = (year) => async(dispatch) => {
    dispatch({type: ORDER_INCOME_PER_MONTH_LIST_REQUEST});
    try {
        const {data} = await axios.get(`/api/admin/order/permonth/${year}`);
        dispatch({type: ORDER_INCOME_PER_MONTH_LIST_SUCCESSFUL, payload: data});
    } catch (error) {
        const message = error.response && error.response.data.message
                        ? error.response.data.message
                        : error.message;
        dispatch({type: ORDER_INCOME_PER_MONTH_LIST_FAILED, payload: message});
    }
};
